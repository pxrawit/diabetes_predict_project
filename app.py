...
def load_model():
    ...
    le_smoking = joblib.load('label_encoder_smoking.pkl')
    return model, scaler, le_gender, le_smoking

model, scaler, le_gender, le_smoking = load_model()
...
# ==================== MODEL INFO ====================
    ...
    **Random Forest** เป็น Ensemble Learning Method ที่สร้าง Decision Trees จำนวนมาก
    ...
    # โหลดข้อมูลเพื่อคำนวณ metrics
    df = pd.read_csv('diabetes_prediction_dataset.csv')
    df['gender'] = le_gender.transform(df['gender'])
    df['smoking_history'] = le_smoking.transform(df['smoking_history'])
